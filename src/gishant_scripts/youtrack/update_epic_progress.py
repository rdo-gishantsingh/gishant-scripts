...
class YouTrackEpicProgressUpdater:
        ...
        for child in children:
            ...
            if "core architecture" in summary or "project setup" in summary:
                ...
                if analysis["cli_commands"] and analysis["services"]:
                    status = "✅ Completed"
                    details.append(f"Project structure set up with {analysis['summary']['total_cli_files']} CLI modules")
                    ...
                    if service_name in summary or service_name.replace("_", " ") in summary:
                        if status == "❌ Not Started":
                            if service_info['size'] > 4000:
                                ...
                                status = "🚧 In Progress"
                        details.append(f"Service implemented in `{service_info['file']}` ({service_info['size']} bytes)")
    ...
    def display_dry_run_preview(
        ...
        self.console.print(
            f"[yellow]💡 {len(mapping)} comments ready to post. "
            "Use --no-dry-run to actually post them.[/yellow]"
        )
