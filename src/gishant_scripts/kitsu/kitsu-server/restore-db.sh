...
if [[ "$BACKUP_FILE" == *.dump ]] || [[ "$BACKUP_FILE" == *.backup ]]; then
    ...
    # pg_restore often returns exit code 1 on harmless warnings, so we allow it but check output manually if needed
    docker compose exec -T $DB_SERVICE pg_restore -U $DB_USER -d $DB_NAME -v --no-owner --no-acl "/tmp/restore.dump" || true
